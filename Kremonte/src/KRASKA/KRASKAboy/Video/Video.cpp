...
namespace KRASKA {
    ...
    Video::Video(KRASKAboy& inGb, Options& inOptions) :
        gb(inGb),
        buffer(KRASKABOY_WIDTH, KRASKABOY_HEIGHT),
    ...
    void Video::draw_bg_line(uint current_line) {
        ...
        for (uint screen_x = 0; screen_x < KRASKABOY_WIDTH; screen_x++) {
    ...
    void Video::draw_window_line(uint current_line) {
        ...
        if (scrolled_y >= KRASKABOY_HEIGHT) { return; }
    ...
    auto Video::is_on_screen_x(u8 x) -> bool { return x < KRASKABOY_WIDTH; }

    auto Video::is_on_screen_y(u8 y) -> bool { return y < KRASKABOY_HEIGHT; }
